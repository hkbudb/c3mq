...
pub fn write_graph_q2_pre_info(which_dataset: u8, secret: <Bls12_381 as Pairing>::ScalarField) {
    let (table, r1_anno) = load_data(which_dataset);
    let mut r1_to = table.get_col(1);
    let mut r2_from = table.get_col(0);
    let r2_anno = r1_anno.clone();
    let mut r3_from = table.get_col(0);
    let r3_anno = r1_anno.clone();
    let r4_from = table.get_col(0);
    let r4_to = table.get_col(1);

    let mut pre_infos = vec![];

    // first psi, xs: r1_to, ys: r4_from
    // local group by
    ...
    let ys = remove_duplicate(&r4_from);
    ...
    // second psi, xs: r2_from, ys: r4_to
    // local group by
    let mut rng = get_fixed_rng();
    let _ = Relation::local_add_agg(&mut r2_from, &r2_anno, &mut rng);
    let n_psi = table.get_size();
    let max_deg = n_psi - 1;
    let mut kzg = setup_kzg(max_deg);
    kzg.setup(secret);
    // remove duplicates in ys
    let ys = remove_duplicate(&r4_to);
    let xs_pts = points_from_num(r2_from);
    let xs_info = kzg.pre_compute_g1(&xs_pts, secret);
    let ys_pts = points_from_num(ys);
    let veri_info = kzg.pre_compute_g1(&ys_pts, secret);
    let z_commit = kzg.pre_compute_z_commitment(&ys_pts, secret);
    ...
    let file_path = if which_dataset == 0 {
        Path::new(GRQC_DATA_PATH).join("q2_pre_info.json")
    } else if which_dataset == 1 {
        Path::new(BTC_DATA_PATH).join("q2_pre_info.json")
    } else if which_dataset == 2 {
        Path::new(P2P_DATA_PATH).join("q2_pre_info.json")
    } else if which_dataset == 3 {
        Path::new(WIKI_DATA_PATH).join("q2_pre_info.json")
    } else {
...
pub(crate) fn load_graph_q2_pre_info(which_dataset: u8) -> Vec<u8> {
    let file_path = if which_dataset == 0 {
        Path::new(GRQC_DATA_PATH).join("q2_pre_info.json")
    } else if which_dataset == 1 {
        Path::new(BTC_DATA_PATH).join("q2_pre_info.json")
...
pub fn exe_graph_q2_batch(process_id: u16, debug: bool, query_num: usize, which_dataset: u8) {
    ...
    let (a1, a2, b1, b2, c1, c2) = obtain_beaver_tripes_by(out_len, triples);
     ...
     let bytes = load_graph_q2_pre_info(which_dataset);
    ...
    let (mut stream_1_4, mut stream_4_1) = UnixStream::pair().unwrap();
    let (mut stream_2_4, mut stream_4_2) = UnixStream::pair().unwrap();
    let (mut stream_3_4, mut stream_4_3) = UnixStream::pair().unwrap();
    ...
    let n_2nd_psi = r2_from.len();
    let max_deg_2nd = n_2nd_psi - 1;
    let n_3rd_psi = r3_from.len();
    ...
    // thread_2
    let mut kzg_2 = setup_kzg(max_deg_2nd);
    kzg_2.setup(secret);

    // thread_3
    let mut kzg_3 = setup_kzg(max_deg_3rd);
    kzg_3.setup(secret);

    // thread_4
    let mut kzg_4_1st = setup_kzg(max_deg_1st);
    kzg_4_1st.setup(secret);
    let mut kzg_4_2nd = setup_kzg(max_deg_2nd);
    kzg_4_2nd.setup(secret);
    let mut kzg_4_3rd = setup_kzg(max_deg_3rd);
    kzg_4_3rd.setup(secret);
    ...
    let thread_2 = std::thread::spawn(move || {
        let pool = ThreadPoolBuilder::new().num_threads(THREAD_NUM).build().unwrap();
        pool.install(|| {
            let self_id = process_id + N;
            // second psi sender
            let mut rng = get_fixed_rng();
            let (r2_anno_sorted, _) = Relation::local_add_agg(&mut r2_from, &r2_anno, &mut rng);
            let xs_pts = points_from_num(r2_from.clone());
            let xs_tree = SubProductTree::new_from_points_parallel(&xs_pts);
            let share1s_2nd = batch_psi_anno_sender_parallel(
                vec![r2_from.into_iter().map(|x| x as u64).collect(); query_num],
                vec![r2_anno_sorted.into_iter().map(|x| x as u64).collect(); query_num],
                &kzg_2,
                &mut stream_2_4,
            ...
            send_data_by_stream(&mut stream_2_3, &bytes, false);
        });
    });
    ...
    let thread_3 = std::thread::spawn(move || {
        ...
        pool.install(|| {
            ...
            let share1s_3rd = batch_psi_anno_sender_parallel(
            ...
            let bytes = bincode::serialize(&share1s_3rd).unwrap();
            let dig = digest(Algorithm::SHA256, &bytes);
            if !debug {
                run_consensus(dig, self_id);
            }
            ...
            let bytes = receive_data_by_stream(&mut stream_3_2);
            let share1s_2nd: Vec<Vec<u128>> = bincode::deserialize(&bytes).unwrap();

            let res1s_1: Vec<Vec<u128>> = share1s_1st.iter().zip(share1s_2nd.iter()).map(|(share1_1st, share1_2nd)| {
                vec_mul_1(
                    share1_1st,
                    share1_2nd,
                    &a1,
            ...
            let res1s: Vec<Vec<u128>> = res1s_1.iter().zip(share1s_3rd.iter()).map(|(res1_1, share1_3rd)| {
                vec_mul_1(
                    res1_1,
                    share1_3rd,
                    &a1,
            ...
            // reveal res1s to thread 4
            let bytes = bincode::serialize(&res1s).unwrap();
            send_data_by_stream(&mut stream_3_4, &bytes, false);
        });
    });
    ...
    let thread_4 = std::thread::spawn(move || {
        let pool = ThreadPoolBuilder::new().num_threads(THREAD_NUM).build().unwrap();
        pool.install(|| {
            let self_id = process_id + 3 * N;
            // first psi receiver
            let share2s_1st = batch_psi_anno_receiver_parallel(
                &vec![r4_from.clone(); query_num],
                true,
            ...
            // third psi receiver
            let share2s_3rd = batch_psi_anno_receiver_parallel(
                &vec![r4_to.clone(); query_num],
                true,
                vec![ys_3rd_no_dup.clone().into_iter().map(|x| x as u64).collect(); query_num],
                &kzg_4_3rd,
                &mut stream_4_3,
            ...
            let res2s_1: Vec<Vec<u128>> = share2s_1st.iter()
            ...
            let res2s: Vec<Vec<u128>> = res2s_1.iter()
            .zip(share2s_3rd.iter())
            .map(|(res2_1, share2_3rd)| {
                vec_mul_2(
                    res2_1,
                    share2_3rd,
                    &a2,
            ...
            let bytes = receive_data_by_stream(&mut stream_4_3);
...
pub fn exe_graph_q2_single(process_id: u16, debug: bool, intra: bool, which_dataset: u8) {
    println!("loading dataset...");
    let (table, r1_anno) = load_data(which_dataset);
    let mut r1_to = table.get_col(1);
    let mut r2_from = table.get_col(0);
    ...
    let thread_2 = std::thread::spawn(move || {
        ...
        pool.install(|| {
            ...
            let share1_2nd = if intra {
                intra_psi_anno_sender_parallel(
                    r2_from.into_iter().map(|x| x as u64).collect(),
                    r2_anno_sorted.into_iter().map(|x| x as u64).collect(),
                    &kzg_2,
                    &mut stream_2_4,
                    &xs_info_2,
                    &xs_tree,
    ...
    let thread_3 = std::thread::spawn(move || {
        ...
        pool.install(|| {
            ...
            // consensus on share1_1st
            let bytes = bincode::serialize(&share1_3rd).unwrap();
            let dig = digest(Algorithm::SHA256, &bytes);
            if !debug {
                run_consensus(dig, self_id);
            }
            ...
            let bytes = receive_data_by_stream(&mut stream_3_2);
            let share1_2nd: Vec<u128> = bincode::deserialize(&bytes).unwrap();

            let res1_1 = vec_mul_1(&share1_1st, &share1_2nd, &a1, &b1, &c1, &mut stream_3_4);
            let res1 = vec_mul_1(&res1_1, &share1_3rd, &a1, &b1, &c1, &mut stream_3_4);
        ...
        pool.install(|| {
            ...
            let share2_1st = if intra {
                ...
                basic_psi_anno_receiver_parallel(
                    r4_from.clone().into_iter().map(|x| x as u64).collect(),
                    &mut stream_4_1,
            ...
            let share2_2nd = if intra {
                ...
                basic_psi_anno_receiver_parallel(
                    r4_to.clone().into_iter().map(|x| x as u64).collect(),
                    &mut stream_4_2,
            ...
            // second psi receiver
            let share2_3rd = if intra {
                intra_psi_anno_receiver_parallel(
                    &r4_to,
                    true,
                    ys_3rd_no_dup.clone().into_iter().map(|x| x as u64).collect(),
                    &kzg_4_3rd,
                    &mut stream_4_3,
                    &ys_tree_3rd,
                    &vec![veri_info_3],
            ...
            let res2_1 = vec_mul_2(&share2_1st, &share2_2nd, &a2, &b2, &c2, &mut stream_4_3);
            let res2 = vec_mul_2(&res2_1, &share2_3rd, &a2, &b2, &c2, &mut stream_4_3);
