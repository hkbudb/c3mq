use fancy_garbling::{
    errors::TwopacError, WireLabel, ArithmeticWire, 
    // Fancy, FancyArithmetic, FancyBinary, FancyReveal, 
    WireMod2,
};
use rayon::iter::{IntoParallelRefIterator, ParallelIterator};
use super::{inner_gb::Garbler as Gb, traits::*};
...
impl<
        C: AbstractChannel,
        RNG: CryptoRng + Rng + SeedableRng<Seed = Block> + Clone,
    ...
    > FancyInput for Garbler<C, RNG, OT, Wire>
{
    ...
    fn receive_many(&mut self, qs: &[u16]) -> Result<Vec<Wire>, TwopacError> {
        // println!("gb: receive_many");
        self.channel.flush()?;
        let n = qs.len();
        let lens = qs.par_iter().map(|q| f32::from(*q).log(2.0).ceil() as usize);
...
impl<C: AbstractChannel, RNG: CryptoRng + Rng + Clone, OT> FancyBinary for Garbler<C, RNG, OT, WireMod2> {
...
impl<C: AbstractChannel, RNG: CryptoRng + Rng + Clone, OT, Wire: WireLabel + ArithmeticWire> FancyArithmetic
...
impl<C: AbstractChannel, RNG: CryptoRng + Rng + Clone, OT, Wire: WireLabel> Fancy
    for Garbler<C, RNG, OT, Wire>
{
    type Item = Wire;
    type Error = TwopacError;
...
impl<C: AbstractChannel, RNG: CryptoRng + Rng + Clone, OT, Wire: WireLabel> FancyReveal
    for Garbler<C, RNG, OT, Wire>
{
