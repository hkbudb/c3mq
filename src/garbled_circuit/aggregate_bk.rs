...
pub fn gb_agg<C>(rng: &mut AesRng, channel: &mut C, ind: &Vec<u128>, anno1: &Vec<u128>, tp: AggType)
where
    C: AbstractChannel + std::clone::Clone,
{
    let mut gb =
        Garbler::<C, AesRng, OtSender, AllWire>::new(channel.clone(), rng.clone()).unwrap();
    let circuit_wires = gb_set_agg_inputs(&mut gb, ind, anno1);

    match tp {
        AggType::Add => {
            let add_agg =
                fancy_add_agg::<Garbler<C, AesRng, OtSender, AllWire>>(&mut gb, circuit_wires)
                    .unwrap();
            for agg in add_agg {
        ...
        AggType::Or => {
            let or_agg =
                fancy_or_agg::<Garbler<C, AesRng, OtSender, AllWire>>(&mut gb, circuit_wires)
                    .unwrap();
            for agg in or_agg {
                gb.outputs(agg.wires()).unwrap();
            }
    ...
    match tp {
        AggType::Add => {
            let (add_agg, next_z0_share2) = fancy_add_agg_small::<
                Garbler<C, AesRng, OtSender, AllWire>,
        ...
        AggType::Or => {
            let (or_agg, next_z0_share2) =
                fancy_or_agg_small::<Garbler<C, AesRng, OtSender, AllWire>>(&mut gb, circuit_wires)
                    .unwrap();
            for agg in or_agg {
                gb.outputs(agg.wires()).unwrap();
            }
...
fn gb_set_agg_inputs<F, E>(gb: &mut F, ind: &Vec<u128>, anno1: &Vec<u128>) -> AggInputs<F::Item>
where
    ...
    C: AbstractChannel + std::clone::Clone,
{
    ...
    match tp {
        AggType::Add => {
            let add_agg =
                fancy_add_agg::<Evaluator<C, AesRng, OtReceiver, AllWire>>(&mut ev, circuit_wires)
                    .unwrap();
            let mut res = vec![];
            for agg in add_agg {
                let val_bin = ev
        ...
        AggType::Or => {
            let or_agg =
                fancy_or_agg::<Evaluator<C, AesRng, OtReceiver, AllWire>>(&mut ev, circuit_wires)
                    .unwrap();
            let mut res = vec![];
            for agg in or_agg {
                let val_bin = ev
    ...
    C: AbstractChannel + std::clone::Clone,
{
    ...
    match tp {
        AggType::Add => {
            let (add_agg, next_z0_share2) = fancy_add_agg_small::<
                Evaluator<C, AesRng, OtReceiver, AllWire>,
    ...
    F: FancyReveal + Fancy + BinaryGadgets + FancyBinary + FancyArithmetic,
{
    ...
    for (idx, ind) in inds.iter().enumerate() {
        ...
        // let sub = f.bin_subtraction(&one, &ind)?.0;
        // let z_i_prim = f.bin_mul(&z_i, &sub)?;
        res.push(z_i_prim);
...
pub fn gb_agg_fast(chunk_size: usize, ind: Vec<u128>, anno1: Vec<u128>, stream: UnixStream, tp: AggType) {
    // next_z0_share1 should be a random val, here just for performance test
    let next_z0_share1 = 0_u128;
    let inds = split(ind, chunk_size);
...
pub fn ev_agg_fast(chunk_size: usize, anno2: Vec<u128>, stream: UnixStream, tp: AggType) -> Vec<u128> {
...
mod test {
    ...
    fn test_add_agg() {
        ...
        std::thread::scope(|s| {
            s.spawn(|| {
                ...
                let mut channel = Channel::new(reader, writer);
                gb_agg(&mut rng_gb, &mut channel, &ind, &anno1, AggType::Add);
            });
    ...
    */
    ...
    fn test_or_agg() {
        ...
        std::thread::scope(|s| {
            s.spawn(|| {
                ...
                let writer = BufWriter::new(sender);
                let mut channel = Channel::new(reader, writer);
                gb_agg(&mut rng_gb, &mut channel, &ind, &anno1, AggType::Or);
            });
    ...
    fn test_add_agg_small_clear() {
        ...
        let anno1: Vec<u128> = gen_random_unsorted::<u32>(n)
            .into_iter()
            .map(|v| v as u128)
            .collect();
        let anno2: Vec<u128> = gen_random_unsorted::<u32>(n)
            .into_iter()
            .map(|v| v as u128)
            .collect();
        ...
        let indicators = split(ind, chunk_size);
        let anno1s = strict_split(anno1, chunk_size);
        let anno2s = strict_split(anno2, chunk_size);
        let mut res2 = vec![];
    ...
    fn test_fast_add_agg() {
        ...
        let thread1 = std::thread::spawn(move || {
            gb_agg_fast(chunk_size, ind, anno1, sender, AggType::Add);
        });
    ...
    fn test_fast_or_agg() {
        ...
        let (tx, rx) = std::sync::mpsc::channel();
        let thread1 = std::thread::spawn(move || {
            gb_agg_fast(chunk_size, ind, anno1, sender, AggType::Or);
        });
